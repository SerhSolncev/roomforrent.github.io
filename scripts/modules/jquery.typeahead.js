...
(function (factory) {
    ...
    Typeahead.prototype = {
        ...
        buildCancelButtonLayout: function () {
            ...
            const tabindex = this.node[0].getAttribute('tabindex')
            scope.container.find('.typeahead__cancel-button').attr('aria-label', 'נקה שדה').prop('aria-label', 'נקה שדה')
            scope.container.find('.typeahead__cancel-button').attr('tabindex', tabindex).prop('tabindex', tabindex)
            scope.container.find('.typeahead__cancel-button').attr('type', 'button').prop('type', 'button')

            scope.container.find('.typeahead__cancel-button').on('keydown', function (e) {
              if(e.keyCode === 13) {
                e.stopImmediatePropagation();
                e.preventDefault();
